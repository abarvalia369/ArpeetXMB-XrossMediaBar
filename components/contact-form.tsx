...
export function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!supabase) {
      setStatus({ text: "Contact form is not connected yet (missing Supabase env vars). Email me directly below instead.", kind: "error" });
      return;
    }
    ...
    if (error) {
      setStatus({ text: "Couldn't send right now — please try again, or email me directly below.", kind: "error" });
      return;
    }
  ...
  return (
    <form id="form" onSubmit={handleSubmit} className="scroll-mt-[calc(var(--xmb-band-h)+16px)] space-y-5" noValidate>
      <Field id="contact-name" label="Name" error={errors.name}>
        <input
          ...
          className="w-full rounded-lg border border-white/15 bg-white/5 px-4 py-3 text-sm text-white focus:border-white/50"
        />
      </Field>
      ...
      <div className="flex items-center gap-4 pt-1">
        <button
          type="submit"
          disabled={submitting}
          className="rounded-full bg-white px-6 py-3 text-sm font-semibold text-black transition-transform hover:-translate-y-0.5 disabled:opacity-50"
        >
